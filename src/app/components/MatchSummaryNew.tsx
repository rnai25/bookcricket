...
export function MatchSummaryNew({ gameState, onRematch, onNewGame }: MatchSummaryProps) {
  ...
  const getKeyMoments = () => {
    ...
    // Add toss result
    moments.push(`${gameState.tossWinner.name} won the toss and elected to ${gameState.tossWinner.id === gameState.battingFirst.id ? 'bat' : 'bowl'} first`);
    ...
    if (firstInningsSixes > 0) {
      moments.push(`${gameState.battingFirst.name} hit ${firstInningsSixes} sixes in their innings`);
    }
    if (secondInningsSixes > 0) {
      moments.push(`${gameState.bowlingFirst.name} hit ${secondInningsSixes} sixes in their innings`);
    }
    ...
    <div className="space-y-8">
      <div className="bg-emerald-800/70 rounded-lg border border-emerald-700/50 p-6">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <h3 className="text-yellow-400 font-semibold mb-4">{gameState.battingFirst.name}&apos;s Innings</h3>
          ...
          <div>
            <h3 className="text-yellow-400 font-semibold mb-4">{gameState.bowlingFirst.name}&apos;s Innings</h3>
            <div className="space-y-2 text-emerald-200">
